...
if __name__=="__main__":
    ...
    import os
    eval_dataset=VOCDataset("/root/VOC2007",resize_size=[800,1024],split='train')
    ...
    for img,boxes,classes in eval_loader:
        ...
        for i,box in enumerate(boxes):
            ...
            img_pad=cv2.rectangle(img,pt1,pt2,(0,255,0))
            img_pad=cv2.putText(img,"%s %.3f"%(COCODataset.CLASSES_NAME[int(classes[i])],scores[i]),(int(box[0]),int(box[1])+10),cv2.FONT_HERSHEY_SIMPLEX,0.5,[0,200,20],2)
        cv2.imwrite("/root/FCOS.Pytorch-master/picture1/"+str(ind),img_pad)
        ind+=1
    '''
    root="/root/VOC2007/JPEGImages/"
    ...
    for name in names:
        ...
        start_t=time.time()
        with torch.no_grad():
            print(img1.unsqueeze_(dim=0).shape)
            out=model(img1.cuda())
        end_t=time.time()
        ...
        cv2.imwrite("/root/FCOS.Pytorch-master/picture1/"+name,img_pad)
    '''
